...
#scraping each profile of professor
def parse_profile_metrics(url):
    ...
    #TOTAL PUBLICATIONS (RESTORED)
    pub = soup.select_one("ul.faculty_profile_count li a")
    if pub:
        text_val = pub.get_text(strip=True) 
        num = text_val.split()[0]
        if num.isdigit():
            data["total_publiclications"] = int(num)
    else:
        m_pub = re.search(r"Publications\s*\((\d+)\)", text)
        if m_pub:
            data["total_publiclications"] = int(m_pub.group(1))

    #CITATIONS
    m_cit = re.search(r"(\d+)\s+Citations", text)
    if m_cit:
        data["citations_box"] = int(m_cit.group(1))
    ...
    gs = soup.select_one("#google_scholar_citation span.counter.cit_count.p0")
    if gs:
        try:
            data["google_scholar_citations"] = int(gs.get_text(strip=True))
        except:
            pass
    ...
    i10_title = soup.find(lambda tag:
        tag.name in ("h4", "h5")
        and "10" in tag.get_text()
        and "index" in tag.get_text().lower()
    )
